...
\red{This requires further investigation.}
...
For instance, \textit{Renter Landscape} (renter share, rent burden), \textit{Housing} 
(structure age, structure size, household age), or \textit{Energy} (energy burden, heating fuel).
The only real difference currently is between variables related to housing age and those not related
to housing age.
The reason is as follows.

Far enough back in the past, ACS5 variables \verb|B25036| count numbers of structures built in a given decade.
For instance, in the 2024 ACS5 \verb|B25036| counts structures built pre-1939,
from 1940-1949, from 1950-1959, and so on.
Things change when considering the more recent past.
The 2024 ACS5 data, \verb|B25036| counts structures built 2010-2019, and then 2020-2024.
On the other hand, though, in 2019, \verb|B25036| counts structures built 
2010-2013, and then 2014-2019.

This creates a mismatch that would require \textit{lots} of manual lookups and rewrites
if one wanted to reuse code on different years.
The functions
\begin{itemize}
    \item \verb|get_b25036_metadata()|
    \item \verb|parse_construction_range()|
    \item \verb|get_relevant_variables()|
\end{itemize}
address this mismatch, and are helpers for \verb|get_housing_size_data()|.
The helpers grab metadata for a given year, parse the metadata, and return a dictionary
of appropriately-named variables for the given year.
For example, 
\begin{verbatim}
    get_relevant_variables(2024)
        {'B25036_014E': 'built_2020_to_2024',
        'B25036_015E': 'built_2010_to_2019',
        'B25036_016E': 'built_2000_to_2009',
        'B25036_017E': 'built_1990_to_1999',
        'B25036_018E': 'built_1980_to_1989',
        'B25036_019E': 'built_1970_to_1979',
        'B25036_020E': 'built_1960_to_1969',
        'B25036_021E': 'built_1950_to_1959',
        'B25036_022E': 'built_1940_to_1949',
        'B25036_023E': 'built_pre_1939'}
\end{verbatim}
...
\subsection*{Style Considerations}
In order to return standardized products, adhere to the following style/color standards.
\textbf{These are an invention of me, Caleb Kennedy Hill, and are by no means ``correct.''}
But it's what we're doing so far.

Some data are transformed from continuous numbers to classes. 
E.g., 1.369 is ``low,'' 6.893 is ``medium,'' and 10.733 is ``high.''
For cases when AGOL is able to do this automatically, the color scheme 
\verb|Purple 1| should be used for NEMS members, and \verb|Green 5| for non-NEMS members.
When AGOL cannot automatically do this, use the following table for colors:

\begin{center}
\begin{tabular}{c||ccc}
    & Low & Medium & High \\ \hline \hline
    NEMS & \verb|# eecec9| & \verb|# d588b9| & \verb|# a14e96| \\
    non-NEMS & \verb|# e2ebd3| & \verb|# add46a| & \verb|# 2d8078|
\end{tabular}
\end{center}

For example, in the Housing Size map, we translate Low, Medium, and High to mean 1, 2-4, and 5+ units, respectively.
...
\begin{enumerate}
    ...
    \blue{\textbf{NOTE:} }
    For almost every data point, we are delivering three different views: NEMS, non-NEMS, and all.
    Thus we need to create one Web Map with the filter set to 
    \verb|nems=1|, one Web Map with \verb|nems=0|, and one with no filter on that field.
    Repeat all previous steps for each of these maps.
